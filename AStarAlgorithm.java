...
public class AStarAlgorithm {
	...
	public int[][] initializeTempGraph() {
		...
		for (int i = 0; i < 3; i++) {
			for (int j = 0; j < 3; j++) {
				g[i][j] = graphQueue.get(graphQueue.size()-1).state[i][j];
			}
		}
	...
	public void algorithmStep(int s) { // step is g(x)
		...
		if (min == 0)
			g = moveLeft(graphQueue.get(graphQueue.size()-1).state);
		else if (min == 1)
			g = moveRight(graphQueue.get(graphQueue.size()-1).state);
		else if (min == 2)
			g = moveUp(graphQueue.get(graphQueue.size()-1).state);
		else if (min == 3)
			g = moveDown(graphQueue.get(graphQueue.size()-1).state);
